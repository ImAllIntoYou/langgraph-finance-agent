...
llm_name = "gemini-2.0-flash-001"   # "gemini-2.0-pro-exp-02-05"

model = ChatGoogleGenerativeAI(api_key=gemini_key, model="gemini-2.0-flash-001")
...
def gather_financials_node(state: AgentState):
    ...
    # Possible optimization: use a "small_model" here for initial data preprocessing 
...
def research_competitors_node(state: AgentState):
    ...
    # Instead of a loop, we can do parallel processing by compiling
    # it into fetch_competitor_data() and call ThreadPoolExecutor()
    for competitor in state["competitors"]:
        ...
        for q in queries.queries:
            # The tavily search can be put into a TTL cache (from cachetools import TTL cache)
            # in order to avoid frequently accessed data to become redundant
